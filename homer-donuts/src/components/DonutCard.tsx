...
import { Heart, Star, Plus, Minus, ShoppingBag, Sparkles } from "lucide-react";
import { playCoinSound, playFavoritedSound, playPopSound } from "./SoundEffects";
...
export default function DonutCard({ donut, onAddToCart, isFavorite, onToggleFavorite }: DonutCardProps) {
  ...
  // Turn favorite state with animation
  const toggleFav = () => {
    playFavoritedSound();
    setAnimateHeart(true);
    onToggleFavorite(donut.id);
  ...
  return (
    ...
    >
      ...
      <button
        onClick={toggleFav}
        className="absolute top-4 right-4 z-20 bg-cream-yellow p-2.5 rounded-full cartoon-border-sm cartoon-shadow-sm hover:bg-pink-100 transition-colors cursor-pointer group"
        aria-label="Favoritar"
      >
        <motion.div
          animate={animateHeart ? { scale: [1, 1.4, 0.9, 1.1, 1] } : {}}
          transition={{ duration: 0.4 }}
        >
          <Heart
            className={`w-5 h-5 transition-transform group-hover:scale-110 ${
              isFavorite ? "fill-pink-primary text-pink-primary" : "text-choco-main"
            }`}
          />
        </motion.div>
      </button>
