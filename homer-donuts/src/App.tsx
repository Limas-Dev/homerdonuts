...
export default function App() {
  ...
  // Fetch real-time stock on mount and poll every 5 seconds for absolute device consistency
  useEffect(() => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setDonuts((prevDonuts) => {
            const updated = prevDonuts.map((d) => ({
              ...d,
              stock: stockMap[d.id] !== undefined ? stockMap[d.id] : d.stock,
            }));
            localStorage.setItem("donut_products_stock", JSON.stringify(updated));
            return updated;
          });
        }
      } catch (e) {
        console.error("Erro ao sincronizar estoque com o servidor:", e);
      }
    };
  ...
  const handleOrderSubmitted = (orderData: {
    ...
    // Decrease stock levels based on the submitted order's items (optimistic local update)
    setDonuts((prevDonuts) =>
      prevDonuts.map((d) => {
    ...
    // Call server database endpoint to deduct stock levels atomically on the backend
    const itemsToPurchase = orderData.items.map((it) => {
      const matchedDonut = donuts.find((d) => d.name.toLowerCase() === it.donutName.toLowerCase());
      return {
        id: matchedDonut ? matchedDonut.id : "",
    ...
    fetch("/api/stock/purchase", {
      ...
      console.error("Erro ao registrar compra de estoque no servidor:", err);
    });
  };
  ...
  return (
    <div className="min-h-screen bg-[#faf6eb] pb-20 relative select-none">
      ...
      {/* Styled Header Component with basket state */}
      <Header
      ...
      <footer className="w-full py-8 mt-12 bg-white border-t-4 border-choco-main text-center text-choco-light font-bold text-xs select-none">
        ...
        >
          <span>🔐</span> Área Exclusiva do Dono (Controle de Estoque)
        </button>
      </footer>
      ...
      {isOwnerOpen && (
        <OwnerDashboard
          ...
          onUpdateStock={async (id, newStock) => {
            ...
            // Write update to persistent server database
            try {
              ...
              console.error("Erro ao atualizar estoque no servidor:", e);
            }
          }}
          onClose={() => {
            playPopSound();
